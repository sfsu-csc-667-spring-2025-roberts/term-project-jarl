...
// Apply session to HTTP routes
config.session(app, io); // ✅ correct
app.use(middleware.room);
config.liveReload(app);
config.sockets(io, app);

// Session typing for express-session
declare module "express-session" {
  interface SessionData {
    userId?: number;
  }
...
// Setup socket for friends feature
setupFriendSocket(io);
...
// Static and view settings
app.use(express.static(path.join(process.cwd(), "public")));
app.set("views", path.join(process.cwd(), "views"));
app.set("view engine", "ejs");

// Apply fallback session for routes needing it
