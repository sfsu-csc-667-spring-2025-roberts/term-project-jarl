...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({
        error:
          "Failed to join game. Check if the game is full, already started, or if you need a password.",
      });
    }
  },
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (game.is_started) {
        return res
          .status(400)
          .json({ error: "Cannot leave a game that has already started" });
      }
      ...
      if (player.is_host) {
        await db.none("UPDATE games SET is_active = false WHERE game_id = $1", [
          gameIdNum,
        ]);
