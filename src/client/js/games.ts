...
function handleGameUpdated(data: any) {
  ...
  const gameItem = document.querySelector(`.game-item[data-game-id="${data.gameId}"]`);
  if (gameItem) {
...
function updateGameUI(game: any) {
  ...
  // Update current turn
  if (game.currentTurn) {
    const isMyTurn = game.currentTurn === document.body.getAttribute('data-user-id');
    const turnElement = document.getElementById('turn-indicator');
    if (turnElement) {
      turnElement.textContent = isMyTurn ? 'Your turn!' : `${game.currentTurn.username || 'Player'}'s turn`;
    }
